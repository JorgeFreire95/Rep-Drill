...
# ✅ NUEVO: Validar que sea empleado
class IsEmployeeUser(permissions.BasePermission):
    """
    ✅ Solo permite acceso a usuarios que sean empleados
    Valida que el usuario tenga un rol de empleado (admin, manager, employee)
    """
    ...
    def has_permission(self, request, view):
        ...
        # Verificar que sea empleado (tiene relación con rol)
        if not hasattr(request.user, 'role') or not request.user.role:
            return False
        
        # Validar que el rol sea de empleado
        allowed_roles = ['admin', 'manager', 'employee']
        return request.user.role.name in allowed_roles


# ✅ NUEVO: Validar que solo empleados puedan crear órdenes
class CanCreateOrder(permissions.BasePermission):
    """
    ✅ Solo empleados con rol EMPLEADO o superior pueden crear órdenes
    """
    
    def has_permission(self, request, view):
        # Si no es POST, permitir a todos los empleados
        if request.method != 'POST':
            return True
        
        # Para POST, validar que sea empleado
        if not request.user or not request.user.is_authenticated:
            return False
        ...
        # Los tres roles pueden crear órdenes
        allowed_roles = ['admin', 'manager', 'employee']
        return request.user.role.name in allowed_roles
